...
import { Home, Users, FolderOpen, Calendar, FileText, PieChart, ChevronRight, Menu, X } from 'lucide-react';
      ...
      >
        ...
        <div className="p-6 flex items-center justify-between">
          <div className={`flex items-center gap-3 transition-all duration-300 ${isCollapsed && !isMobile ? 'opacity-0 w-0 overflow-hidden' : 'opacity-100'}`}>
            ...
            <span className="text-white font-semibold text-lg whitespace-nowrap">Krav Maga Pro</span>
          </div>
          <button
            onClick={toggleSidebar}
            className={`p-2 rounded-lg hover:bg-slate-800/40 transition-all duration-200 text-slate-400 hover:text-white flex-shrink-0 ${
          ...
          <div className="px-4 py-4 border-t border-slate-700/50 space-y-2">
            {teams.map((team) => (
              <button
                key={team.id}
                onClick={() => handleTeamClick(team.id)}
                className={`w-full flex items-center justify-center p-2.5 rounded-lg text-sm font-medium transition-all duration-200 ${
                  activeTeam === team.id
                    ? 'bg-slate-800/80 text-white'
                    : 'text-slate-400 hover:text-white hover:bg-slate-800/40'
                }`}
        ...
        <div className="p-4 border-t border-slate-700/50">
          <button 
            className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-lg hover:bg-slate-800/40 transition-all duration-200 ${
              isCollapsed && !isMobile ? 'justify-center' : ''
            }`}
            ...
            {(!isCollapsed || isMobile) && (
              <>
                <div className="flex-1 text-left">
                  <div className="text-sm font-medium text-white">Tom Cook</div>
